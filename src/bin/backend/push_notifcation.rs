...
use crate::{models::{PushCredential, PushKey}, parse_env_var};

static PEM_FILE: Lazy<File> = Lazy::new(|| File::open(parse_env_var::<String>("PEM_FILE")).unwrap());

pub async fn push_notifcation(username: String, mongoc: &mut mongodb::Client) -> Option<()> {
    let mut cred_cursor: Cursor<PushCredential> = mongoc.default_database().unwrap().collection("push_credentials").find(doc! { "username": username }, None).await.ok()?;
    ...
    while let Ok(true) = cred_cursor.advance().await {
        ...
        let sig_builder = VapidSignatureBuilder::from_pem(PEM_FILE.try_clone().ok()?, &subscription_info).ok()?.build().ok()?;
