...
use crate::{
    config::JWT_KEY, database_client::{init_database, MONGOC}, parse_env_var
};
...
pub async fn iot_auth_handler(Json(body): Json<IotAuthBody>) -> impl IntoApiResponse {
...
pub fn iot_auth_routes() -> ApiRouter {
    ApiRouter::new().api_route("/iot", post_with(iot_auth_handler, |op| {
