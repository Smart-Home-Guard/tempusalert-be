...
macro_rules! create_features {
    ($mongoc:expr, $init_mqtt_client:expr, $($feature_module:ident),*) => {{
        ...
        $(
            ...
            let web_feat = $feature_module::WebFeature::create($mongoc, iot_rx, iot_tx, JWT_KEY.to_owned()).unwrap();
            let (mqttc, event_loop) = $init_mqtt_client($feature_module::IotFeature::name().as_str()).await;
            let iot_feat = $feature_module::IotFeature::create(mqttc, event_loop, $mongoc, web_rx, web_tx, JWT_KEY.to_owned()).unwrap();
            web_features.push(Arc::new(Mutex::new(web_feat)) as Arc<Mutex<dyn WebFeature + Send + Sync>>);
            iot_features.push(Arc::new(Mutex::new(iot_feat)) as Arc<Mutex<dyn IotFeature + Send + Sync>>);
        )*
