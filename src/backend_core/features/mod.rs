...
pub trait IotFeature {
    fn create(web_rx: Arc<Mutex<Receiver<WebNotification>>>, web_tx: &mut Sender<IotNotification>) -> Self where Self: Sized;
...
pub trait WebFeature {
    fn create(web_rx: Arc<Mutex<Receiver<IotNotification>>>, web_tx: &mut Sender<WebNotification>) -> Self where Self: Sized;
